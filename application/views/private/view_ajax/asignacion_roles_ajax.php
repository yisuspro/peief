                ...
                <table id="sample_2" class="table table-striped table-bordered table-hover dt-responsive" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        ...
                        foreach($tabla->result_array() as $r) {
                            echo '<tr>';
                            echo '<th>'.$r['ROLE_name'].'</th>';
                            echo '<th>'.$r['ROLE_shortname'].'</th>';
                            echo '<th>'.$r['ROLE_description'].'</th>';
                            if($this->Role->consultarRolUsuario($id,$r['ROLE_PK'])){
                                echo '<th>'.'<label><input type="checkbox" value="'.$r['ROLE_PK'].'" name="checkboxName"  class="checkbox"/><div class="switch switchOn" id="'.$r['ROLE_PK'].'"></div></label>'.'</th>';
                            echo '</tr>';
                            }else{
                                echo '<th>'.'<label><input type="checkbox" value="'.$r['ROLE_PK'].'" name="checkboxName"  class="checkbox"/><div class="switch" id="'.$r['ROLE_PK'].'"></div></label>'.'</th>';
                                echo '</tr>';
                            }
                        }?>
                    </tbody>
    ...
    $(document).ready(function() {
        $('.switch').click(function() {
            if (!$(this).hasClass('switchOn')) {
                ...
                var tr = this.id;
                $.ajax({
                    url: '<?= base_url() ?>Roles/asignarRolUsuario/' + <?= $id;?> + '/' + tr,
                    type: 'POST',
                    data: tr,
                ...
                $.ajax({
                    url: '<?= base_url() ?>Roles/eliminarRolUsuario/'+<?= $id;?>+'/'+tr,
                    type: 'POST',
                    data: tr,
                    success: function(data, xhr) {
                        $("#sample_1").DataTable().ajax.reload();
