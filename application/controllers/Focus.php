...
class Focus extends CI_Controller{
    ...
    */
    public function listarFocus(){
        ...
        $data =$this->Focu->listar();                       //utiliza el metodo listar() del modelo plan() para traer los datos de todos los planes 
    ...
    */
    public function agregarFocus(){
        ...
        if($this->form_validation->run() === FALSE){                //si se incumple algunade las regla
            ...
            if(!$this->Focu->agregarFocus($data)){                          //utilizacion del metodo agregarPlan() del modelo Plan() para la agregacion de un nuevo plan los datos pertinentes
    ...
    */
    public function eliminarFocus($pk){
        if($res = $this->Focu->eliminar($pk)){                                      //realiza la verificacion y eliminacion del plan
    ...
    */
    public function editarFocus($pk){
        $data=$this->Focu->datosFocus($pk);                                 //verifica por medio del metodo datosPlan() del modelo Plan() si el usuario existe ytrae todos los datos pertinentes al usuario 
    ...
    */
    public function modificarFocus($doc){
        ...
        if($this->form_validation->run() === FALSE){                //si se incumple algunade las regla
            ...
            if(!$this->Focu->modificarFocus($doc,$data)){                          //utilizacion del metodo agregarPlan() del modelo Plan() para la agregacion de un nuevo plan los datos pertinentes
                echo "error";                                               // en caso de  fallar envia un mensaje de
            echo json_encode(array('msg'=> 'Enfoque agregado agregado' ));  //si fue agregado con exito envia el mensaje correspondiente
            }
        }
